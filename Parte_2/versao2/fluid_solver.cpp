...
void lin_solve(int M, int N, int O, int b, float *x, float *x0, float a, float c) {
    float tol = 1e-7; // Tolerance for convergence
    float max_c;      // Maximum change
    int l = 0;        // Solver iterations
    float inv_c = 1.0f / c;
    ...
    do {
        ...
        for (int i = 1; i <= M; i++) {
            for (int j = 1; j <= N; j++) {
                #pragma omp simd reduction(max:max_c)
...
// Assuming IX macro and set_bnd function are defined elsewhere
...
//            float old_x, change;
//
//            // Red-Black Gauss-Seidel iteration (Red phase)
//            #pragma omp for collapse(2) private(old_x, change) reduction(max: max_c) schedule(static)
//            for (int i = 1; i <= M; i++) {
//                for (int j = 1; j <= N; j++) {
