...
#include <immintrin.h>  // For SIMD operations if needed (e.g., AVX)

void lin_solve(int M, int N, int O, int b, float *x, float *x0, float a, float c) {
    const int size = (M + 2) * (N + 2) * (O + 2);  // Assuming x and x0 are 1D arrays of 3D grid points

    // Precompute reciprocal of 'c' for division, which can be more expensive
    const float invC = 1.0f / c;

    // Precompute strides to avoid recalculating during indexing
    const int strideN = N + 2;
    const int strideO = O + 2;

    for (int l = 0; l < LINEARSOLVERTIMES; l++) {
        for (int i = 1; i <= M; i++) {
            int iBase = i * strideN * strideO;
            
            for (int j = 1; j <= N; j++) {
                int jBase = iBase + j * strideO;
                
                for (int k = 1; k <= O; k++) {
                    // Calculate index once
                    int idx = jBase + k;
                    
                    // Access neighbors and update 'x'
                    x[idx] = (x0[idx] + a * (
                            x[idx - strideO] + x[idx + strideO] +   // x[i][j-1][k], x[i][j+1][k]
                            x[idx - strideN * strideO] + x[idx + strideN * strideO] + // x[i-1][j][k], x[i+1][j][k]
                            x[idx - 1] + x[idx + 1]  // x[i][j][k-1], x[i][j][k+1]
                    )) * invC;
                }
            }
        }
